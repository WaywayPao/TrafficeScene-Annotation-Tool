...
class MainWindow_controller(QMainWindow):
    ...
    def save_img(self):
        ...
        self.ui.message_box.setText(f"Note: Image save completed")
        print(f"Save to {self.labeled_rgb_path}")
    ...
    def addZone(self, auto_labeling=False):
        ...
        if self.ego_data is not False and not auto_labeling:
            ...
            self.carla_points_color.append(self.color)
        self.points = []
        self.ui.message_box.setText(
            f"Note: Add zone with label {self.color[::-1]}")
    ...
    def auto_label(self):
        ...
        for i in range(self.ui.combobox_frameNum.count()):
            ...
            for points, c in zip(self.carla_points, self.carla_points_color):
                if self.ego_data is not False:
                    compass, ego_loc = self.ego_data
    ...
    def variant_action(self):
        ...
        if self.init_frame_combobox == False:
            ...
            top_path = os.path.join(
                var_path, 'rgb/lbc_img', self.get_current_frame())
            front_path = os.path.join(
                var_path, 'rgb/front', self.get_current_frame())
            front_seg_path = os.path.join(
                var_path, 'instance_segmentation/ins_front', self.get_current_frame())

            self.labeled_top_path = os.path.join(
                var_path, 'construction_zone/top', self.get_current_frame())
            self.labeled_rgb_path = os.path.join(
                var_path, 'construction_zone/rgb', self.get_current_frame())
            self.labeled_seg_path = os.path.join(
                var_path, 'construction_zone/seg', self.get_current_frame())
            ...
            if os.path.isfile(self.labeled_seg_path) and os.path.isfile(self.labeled_rgb_path):
                self.updated_top_img = cv2.imread(self.labeled_top_path)
                # self.updated_top_img = cv2.resize(self.updated_top_img, (1280, 720), interpolation=cv2.INTER_AREA)
                self.updated_front_img = cv2.imread(self.labeled_rgb_path)
                self.updated_front_seg = cv2.imread(self.labeled_seg_path)
