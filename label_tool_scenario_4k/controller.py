...
class MainWindow_controller(QMainWindow):
    ...
    def update_image_frame(self, point, world_point=None):
        ...
        self.set_image(self.updated_top_img,
                       self.ui.main_image, True, (2048, 2048))
    ...
    def clean(self):
        ...
        path_json = os.path.join(self.main_image_path,
                                 'label', 'top', f'{self.road_id}.json')
        if os.path.isfile(path_json):
            os.remove(path_json)

        path_img = os.path.join(self.main_image_path,
                                'label', 'top', f'{self.road_id}.png')
        ...
        self.set_image(self.updated_top_img,
                       self.ui.main_image, True, (2048, 2048))
        self.ui.message_box.setText(
            f"Note: Clean all instance color and delete labeled image")
    ...
    def save_img(self):
        ...
        cv2.imwrite(os.path.join(path_top, self.current_map),
                    self.updated_top_img)

        self.ui.message_box.setText(
            f"Note: Map {self.current_map} save completed")
    ...
    def get_current_color(self):
        temp = self.ui.combobox_instance_color.currentText().split('(')[
            1].split(')')[0]
    ...
    def set_color_combobox(self):
        self.color_list = {'Corner': (255, 0, 0), 'Road': (0, 255, 0), 'Crosswalk': (
            0, 0, 255), 'Intersection': (255, 255, 0), 'Lane': (255, 0, 255), 'Route': (125, 125, 125)}
        ...
        for i, color in enumerate(self.color_list):
            ...
            self.ui.combobox_instance_color.insertItem(
                i, f"{color}    {self.color_list[color]}")
    ...
    def variant_action(self):
        ...
        if self.init_map_combobox == False:
            ...
            if os.path.isfile(self.labeled_top_path):
                ...
                json_path = os.path.join(self.main_image_path, 'label',
                                         'top', f"{self.road_id}.json")
                json_file = open(json_path)
                info = json.load(json_file)
                json_file.close()
                self.points_by_pixel = info["points_by_pixel"]
                self.carla_points = info["carla_points"]
                self.carla_points_color = info["carla_points_color"]
    ...
    def get_ego_data(self):
        ego_data_path = os.path.join(
            self.main_image_path, self.road_id+'.json')
